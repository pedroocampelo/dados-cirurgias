...
# 
# ARQUIVO = os.environ.get("DASH_EXCEL_PATH", "")
# ABA = os.environ.get("DASH_EXCEL_SHEET", "Página1")
# 
# COL_ANO = "ANO"
# COL_ID = "ID PACIENTE"
# COL_SEXO = "SEXO"
...
# ]
# 
# # Modebar: apenas baixar imagem e tela cheia
# # Observação: "toImage" e "toggleFullscreen" não são removidos, então ficam disponíveis.
# PLOTLY_CONFIG = {
#     "displaylogo": False,
...
#     # Estilo + trava de interações (sem zoom/pan por arrasto)
#     fig.update_layout(
#         height=height,
#         font=dict(size=18),
...
# 
#     if len(g) >= 2:
#         max_row = g.loc[g["Cirurgias"].idxmax()]
#         min_row = g.loc[g["Cirurgias"].idxmin()]
#         fig.add_scatter(
#             x=[max_row[COL_ANO]],
#             y=[max_row["Cirurgias"]],
#             mode="markers",
#             marker=dict(size=14),
#             showlegend=False,
#             hovertemplate="Pico: %{y}<extra></extra>",
#         )
#         fig.add_scatter(
#             x=[min_row[COL_ANO]],
#             y=[min_row["Cirurgias"]],
...
#             marker=dict(size=14),
#             showlegend=False,
#             hovertemplate="Queda: %{y}<extra></extra>",
#         )
#     return fig, g
# 
# def donut_sexo(df: pd.DataFrame):
#     g = df.groupby(COL_SEXO).size().reset_index(name="Cirurgias")
#     g = g.sort_values("Cirurgias", ascending=False)
# 
#     g[COL_SEXO] = g[COL_SEXO].replace({
#         "M": "Masculino",
#         "F": "Feminino",
...
# # =========================
# if not ARQUIVO:
#     st.error("Caminho do arquivo não foi passado. Rode a célula que inicia o streamlit corretamente.")
#     st.stop()
# 
...
#     st.write(f"Este painel apresenta um panorama geral das cirurgias registradas. O período observado vai de {min_geral.strftime('%d/%m/%Y')} a {max_geral.strftime('%d/%m/%Y')}.")
# else:
...
# 
# fig_trend, g_trend = fig_tendencia_ano(df_base)
# show(fig_trend, height=850)
# 
# if len(g_trend) >= 2:
#     max_row = g_trend.loc[g_trend["Cirurgias"].idxmax()]
#     min_row = g_trend.loc[g_trend["Cirurgias"].idxmin()]
#     st.caption(f"O maior volume ocorreu em {int(max_row[COL_ANO])} e o menor em {int(min_row[COL_ANO])}.")
# else:
#     st.caption("A tendência é mostrada a partir do conjunto completo de dados.")
# 
# st.markdown("<hr/>", unsafe_allow_html=True)
# 
...
def wait_port(host="127.0.0.1", port=8501, timeout=45):
    start = time.time()
...
env = os.environ.copy()
env["DASH_EXCEL_PATH"] = os.path.abspath(arquivo)
env["DASH_EXCEL_SHEET"] = "Página1"
...
ok = wait_port("127.0.0.1", 8501, timeout=45)

if not ok:
    print("Streamlit não abriu a porta 8501. Log:")
    for _ in range(160):
    ...
    public_url = ngrok.connect(addr="127.0.0.1:8501", proto="http").public_url
    print("Abra este link para acessar o dashboard:")
    print(public_url)
